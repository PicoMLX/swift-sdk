...
| 11 | [#258](https://github.com/modelcontextprotocol/swift-sdk/pull/258) | — | yonaskolb | `adcb230` (branch `pr/258`) | merged | The convenience `Server.init` accepted `name`/`version`/`title`/`instructions` and built a `Server.Info` that silently dropped `description`, `websiteUrl` and `icons` — fields `Server.Info` already declares (`Server.swift:37-42`) and the 2025-11-25 spec already carries in `serverInfo`, its lifecycle example listing all three. Before this they were reachable only by constructing a `Server.Info` by hand and there was no way to set them through `Server(name:version:…)` at all. Twelve lines, no adaptation. **Additive, not source-breaking:** the three parameters default to `nil` and sit between `title:` and `instructions:`; Swift has no positional form for this initializer, so every existing call site still compiles and still produces an identical `Server.Info`. No return trap either — the parameters are the only new surface, so code written against the fork compiles on upstream the moment #258 lands, and until then the worst case is three arguments upstream does not accept. **Closes an asymmetry rather than opening one:** `Client.init` already forwarded the same three fields to `Client.Info` (`Client.swift:231`), and `Server.init` was the only one left behind; the parameter order adopted here matches it. Ships its own test, which is wire-level rather than storage-level — it drives a real `initialize` through a `Server` over `MockTransport`, decodes the response actually sent, and asserts all six `serverInfo` fields plus `instructions`. Verified in both directions, with the caveat that the negative direction is a **compile** failure, not a failing assertion: on `815617c` the test file does not build (`extra arguments at positions #4, #5, #6`), which is the only failure mode an additive initializer can have. Full suite green on the merge: 587 tests, 43 suites. Carried as authored: `websiteUrl` is an unvalidated `String?`, matching the existing `Server.Info` and `Client.Info` fields and unconstrained by the spec. Tracking: fork issue #22. |
...
The next candidates will come from the five upstream PRs still labelled
`investigate` — #178, #213, #216, #257 and #259 — one of which moves into
this table if investigation promotes it to `merge`.
...
```swift
.package(url: "https://github.com/ianegordon/swift-sdk.git", exact: "0.12.2-ianegordon.11")
```
