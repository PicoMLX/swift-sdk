...
public actor Server {
  ...
  private func finishDispatch(id: ID, token: UUID) {
    ...
    cancellationHandler = TypedNotificationHandler<CancelledNotification> { [weak self] message in
      ...
      // A routing transport maps a client's wire id to the id this server saw; it has
      // no mapping for an id the transport already routed (a stateless HTTP transport
      // rewrites a cancellation's requestId to the exchange id before forwarding it),
      // so fall back to the id as given — the key the request was tracked under. An
      // ambiguous wire id maps to nil and matches nothing either way, so an ambiguous
      // cancellation still cancels nothing.
      let pendingRequestID = await self.routedRequestID(for: requestId) ?? requestId
      let disposition = await self.cancelRequest(id: pendingRequestID)
